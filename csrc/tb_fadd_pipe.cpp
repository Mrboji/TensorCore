...
// 修改测试用例，添加验证逻辑
void test_normal_Add() {
  printf("=== Test Normal Add ===\n");
  
  // 测试 1.5 + 2.0 = 3.5
  // 1.5: sign=0, exp=15+1=16(10000), frac=100
  // 2.0: sign=0, exp=15+1=16(10000), frac=000  
  // 3.5: sign=0, exp=15+2=17(10001), frac=110
  top->a_i = encode_float(0, 15, 0b100);  // 1.5
  top->b_i = encode_float(0, 16, 0b000);  // 2.0
  uint16_t expected_result = encode_float(0, 16, 0b110); // 3.5
  ...
  for (int i = 0; i < 5; i++) {
      single_cycle();
      if (top->out_valid_o) {
          check_result("1.5 + 2.0 = 3.5", expected_result, top->result_o, 0);
          break;
      }
  }
}
...
void test_zero_Add() {
  printf("=== Test Zero Add ===\n");
  
  // 测试 0.0 + 5.0 = 5.0
  top->a_i = encode_float(0, 0, 0);      // 0.0
  top->b_i = encode_float(0, 17, 0b010); // 5.0
  uint16_t expected_result = encode_float(0, 17, 0b010); // 5.0
...
top->in_valid_i = 0;
  ...
  for (int i = 0; i < 5; i++) {
      single_cycle();
      if (top->out_valid_o) {
          check_result("0.0 + 5.0 = 5.0", expected_result, top->result_o, 0);
          break;
      }
  }
}
...
void test_signed_Add() {
  printf("=== Test Signed Add ===\n");
  
  // 测试 -1.5 + 1.0 = -0.5
  top->a_i = encode_float(1, 15, 0b100); // -1.5
  top->b_i = encode_float(0, 15, 0b000); // 1.0
  uint16_t expected_result = encode_float(1, 14, 0b000); // -0.5
  ...
  for (int i = 0; i < 5; i++) {
      single_cycle();
      if (top->out_valid_o) {
          check_result("-1.5 + 1.0 = -0.5", expected_result, top->result_o, 0);
          break;
      }
  }
}
...
void test_infinity_Add() {
  printf("=== Test Infinity Add ===\n");
  ...
  for (int i = 0; i < 5; i++) {
      single_cycle();
      if (top->out_valid_o) {
          check_result("Inf + 2.0 = Inf", expected_result, top->result_o, 0x01);
          break;
      }
  }
}
