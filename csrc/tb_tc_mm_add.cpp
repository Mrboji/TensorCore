...
void set_fp9_matrix(uint16_t data[8][8]) {
...
void set_fp22_matrix(uint32_t data[8][8]) {
    ...
    for (int m = 0; m < SHAPE_M; m++) {
        for (int n = 0; n < SHAPE_N; n++) {
            ...
            // 设置对应的位
            top->matrix_c[word_idx] |= ((uint32_t)value << bit_offset);
            
            // 处理跨32位边界的情况
            if (bit_offset + ELEMENT_WIDTH_C > 32) {
                int remaining_bits = 32 - bit_offset;
                int overflow_bits = ELEMENT_WIDTH_C - remaining_bits;
                top->matrix_c[word_idx + 1] |= (value >> remaining_bits);
            }
        }
    }
}
...
void run_tc_mm_add_test() {
    printf("\n=========================================\n");
    printf("  Testing tc_mm_add Module\n");
    printf("=========================================\n");
    
    // 创建测试数据
    uint16_t matrix_middle[SHAPE_M][SHAPE_N];
    uint32_t matrix_c     [SHAPE_M][SHAPE_N];
    
    // 测试用例：每行的8个元素都是1.0
    for (int n = 0; n < SHAPE_M; n++) {
        for (int k = 0; k < SHAPE_N; k=k+4) {
            // 1.0 = 符号0，指数15，尾数0
            matrix_middle[n][k]   = encode_fp9(0, 16, 0b000);//2.0
            matrix_middle[n][k+1] = encode_fp9(0, 15, 0b100);//1.5
            matrix_middle[n][k+2] = encode_fp9(1, 15, 0b000);//-1.0
            matrix_middle[n][k+3] = encode_fp9(0, 15, 0b100);//1.5

            matrix_c[n][k]   = encode_fp22(0, 127, 0b1000000000000);//1.5
    ...
    set_fp9_matrix(matrix_middle);
    set_fp22_matrix(matrix_c);
